...
window.ATHome=function ATHome(ctx){
 ...
 ${list.length?`<div class="at-h4-lineup-grid ${expanded?'expanded':''}">${tiles.join('')}</div>${list.length>6?`<div class="at-h4-showall"><button type="button" data-home-action="lineup-more" aria-expanded="${expanded}">${expanded?'Shfaq më pak ↑':`Shfaq të gjitha ${list.length} anime ↓`}</button></div>`:''}`:'<div class="at-h2-inline-empty">Nuk ke episode të tjera të transmetuara te “Po shikoj”. Kontrollo kalendarin ose zbulo një anime të re. <button type="button" data-pro-page="recommendations">Zbulo anime →</button></div>'}`;
}
 function releases(){
