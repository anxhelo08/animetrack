...
window.ATHome=function ATHome(ctx){
 ...
 function action(op,id,b){
  ...
  if(op==='mark-next'||op==='advance-next'){const entry=rememberNext(a);if(entry){lastWatch=entry;ctx.markNext(a.id);if(!marked(entry))lastWatch=null;ctx.rerender()}return}
  if(op==='undo-watch'){const old=lastWatch;lastWatch=null;if(old&&old.owner===owner&&marked(old))ctx.undoEpisode?.(old.id,old.seasonId,old.n);else ctx.toast('Nuk ka episod për të zhbërë.');ctx.rerender();return}
  if(op==='more-watching'){limit=Math.min(500,limit+6);ctx.rerender();return}
