...
window.ATUnified119=(()=>{
 ...
 function render(state,opts={}){
  ...
  const matching=tv.filter(s=>(filter==='all'||(filter==='genres'?(genre==='all'||(s.genres||[]).some(g=>g.toLocaleLowerCase()===genre)):['favorites','movies','waiting'].includes(filter)?false:s.status===filter))&&String(s.title||'').toLocaleLowerCase().includes(search));
  ...
  const counts={all:anime.length+tv.length,anime:anime.length,tv:tv.length};
