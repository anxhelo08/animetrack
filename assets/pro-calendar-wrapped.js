window.ATCalendarWrapped=function ATCalendarWrapped(ctx){
 let weekOffset=0,period='month',mode=(typeof matchMedia==='function'&&matchMedia('(max-width:760px)').matches)?'agenda':'week',scope='all',focusDay=null;
 ...
 const dayChips=mode==='agenda'?`<nav class="at113-cal-days" aria-label="Zgjidh ditën">${dates.map(d=>{const id=d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0'),today=dayStart(d).getTime()===dayStart(now).getTime(),chosen=focusDay===id;return `<button type="button" data-pro-action="calendar-day" data-id="${id}" aria-pressed="${chosen}" class="${chosen?'active':today?'today':''}"><small>${esc(d.toLocaleDateString('sq-AL',{weekday:'short'}))}</small><b>${d.getDate()}</b><i>${dayEvents(d).length||'·'} ep.</i></button>`}).join('')}</nav>`:'';
