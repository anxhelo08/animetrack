...
window.ATFriends=function ATFriends(ctx,profiles){
 ...
 function render(){
  ...
  const activity=accepted.flatMap(f=>{
   const person=pById(peer(f));
   const rows=Array.isArray(person.snapshot?.activity)?person.snapshot.activity:[];
   return rows.slice(0,6).filter(x=>x&&typeof x.title==='string'&&Number.isFinite(Date.parse(x.at||''))&&Number.isInteger(Number(x.episode))).map(x=>({person,title:String(x.title).slice(0,180),episode:Number(x.episode),at:Date.parse(x.at)}));
  }).filter(x=>x.at>=Date.now()-30*86400000).sort((a,b)=>b.at-a.at).slice(0,10);
  const feed=`<section class="pro-panel at116-friend-feed"><div class="at11-panel-head"><div><span class="pro-eyebrow">FRIENDS · OPT-IN</span><h3>Aktiviteti i miqve</h3></div><span class="at11-count">${activity.length}</span></div><p class="pro-muted">Shfaqen vetëm episodet që miqtë e pranuar kanë zgjedhur të ndajnë. Aktivizimi bëhet nga profili personal.</p>${activity.length?activity.map(x=>`<article class="at116-friend-activity">${avatar(x.person)}<div><strong>${esc(display(x.person))}</strong><span>${esc(x.title)} · EP ${x.episode}</span><small>${esc(new Date(x.at).toLocaleDateString('sq-AL',{day:'2-digit',month:'short'}))}</small></div></article>`).join(''):'<p class="at11-search-state">Ende nuk ka aktivitet të ndarë nga miqtë. Profili mbetet privat si parazgjedhje.</p>'}</section>`;
