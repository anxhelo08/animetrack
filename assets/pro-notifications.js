...
window.ATNotifications=function ATNotifications(ctx){
 ...
 function collect(){
  ...
  for(const e of ctx.upcoming()){
   ...
   const lead=Number(reminders[key]);if(!Object.prototype.hasOwnProperty.call(reminders,key)||![0,10,30,60,1440].includes(lead)||e.when-lead*60000>now||e.when<now-D||alreadyWatched(e))continue;
   const a=state().anime.find(x=>x.id===e.animeId);if(!a)continue;
   out.push({key:'reminder:'+key,category:'episodes',title:'Kujtesa e episodit',body:a.title+' · EP '+e.episode+' · '+new Date(e.when).toLocaleTimeString('sq-AL',{hour:'2-digit',minute:'2-digit'}),at:e.when-lead*60000,id:a.id,cover:a.cover,season:e.seasonId||'',ep:e.seasonEpisode||e.episode});
  }
  for(const e of ctx.upcoming()){
