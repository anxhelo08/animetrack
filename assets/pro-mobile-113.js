...
window.ATMobile113=(function(){
 ...
 function mountLibrary(){
  ...
  strip.insertAdjacentHTML('beforebegin',`<section id="at113-library-head" class="at113-library-head" aria-label="Përmbledhja e bibliotekës"><div class="at113-library-title"><span>ANIME + TV · LIBRARY</span><h2>Biblioteka ime</h2><p>Progresi yt, i organizuar në një vend.</p></div><div class="at113-library-metrics"><span><b data-at113-count="all">0</b> tituj</span><span><b data-at113-count="watching">0</b> në progres</span><span><b data-at113-count="completed">0</b> përfunduar</span></div><label class="at113-library-search" for="at113-library-search"><span aria-hidden="true">⌕</span><input id="at113-library-search" type="search" inputmode="search" autocomplete="off" placeholder="Kërko në bibliotekën tënde…" aria-label="Kërko në bibliotekë"><span class="at113-search-help" aria-hidden="true">⌕</span></label><div class="at117-library-tools"><span>Renditja</span><button type="button" data-at117-sort="updated">Së fundmi</button><button type="button" data-at117-sort="title">A–Z</button><button type="button" data-at117-sort="progress">Progresi</button><button type="button" data-at117-sort="rating">Vlerësimi</button></div></section>`);
 ...
 document.addEventListener('click',e=>{const button=e.target.closest('[data-at117-sort]');if(!button)return;const sort=$('sort');if(!sort||!['updated','title','progress','rating'].includes(button.dataset.at117Sort))return;sort.value=button.dataset.at117Sort;sort.dispatchEvent(new Event('change',{bubbles:true}));});
 function libraryUpdate(){
  const el=$('at113-library-head');if(!el)return;
  const st=window.ATMobile113?.state?.()||{};const items=[...(st.anime||[]),...(st.tvShows||[])];
