...
window.ATProfiles=function ATProfiles(ctx){
 ...
 function render(){
  ...
  const nav=`<div class="at-profile-tabs" role="group" aria-label="Seksionet e profilit"><button type="button" data-pro-action="profile-tab" data-id="overview" class="${tab==='overview'?'active':''}" aria-pressed="${tab==='overview'}">✦ Përmbledhje</button><button type="button" data-pro-action="profile-tab" data-id="settings" class="${tab==='settings'?'active':''}" aria-pressed="${tab==='settings'}">⚙ Ndrysho profilin</button></div>`;
  const header=`<header class="at-profile-header"><div class="at-profile-banner"><span class="pro-eyebrow">ANIMETRACK · YOUR UNIVERSE</span><span class="at-profile-privacy">${status}</span></div><div class="at-profile-identity"><div class="at-profile-avatar">${avatar?`<img src="${esc(avatar)}" alt="Foto profili" referrerpolicy="no-referrer">`:esc(p.avatar_emoji||'🎌')}</div><div class="at-profile-person"><h2>${esc(name)}</h2><span>${esc(handle)}</span><p>${esc(p.bio||'Çdo anime që shikon është pjesë e historisë tënde.')}</p><div class="at-profile-bits"><span>${joined?'✦ Anëtar që nga '+esc(joined):'✦ Historia jote anime'} </span><span>• ${s.stats.titles} anime në bibliotekë</span></div></div><div class="at-profile-top-actions">${profile?'<button class="pro-btn" data-pro-action="profile-share">↗ Kopjo linkun</button>':''}<button class="pro-btn primary" data-pro-action="profile-tab" data-id="${tab==='overview'?'settings':'overview'}">${tab==='overview'?'Ndrysho profilin':'Shiko profilin'}</button></div></div></header>`;
  ...
  return header+nav+(tab==='settings'?settings:overview);
 }
 function setTab(next){if(!['overview','settings'].includes(next))return;tab=next;ctx.rerender()}
 async function save(){
 ...
 return {load,render,save,share,scheduleSnapshot,snapshot,get:()=>profile,setTab};
};
