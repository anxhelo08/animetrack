...
window.ATProfiles=function ATProfiles(ctx){
 ...
 function snapshot(visibility=profile?.is_public){
  ...
  // Recent watch activity is shared with accepted friends ONLY after explicit opt-in.
  const share=state().preferences?.shareFriendActivity===true&&visibility===false;
  const activity=share?(state().history||[]).slice().reverse().filter(h=>h.action==='watched'&&Date.parse(h.date||'')>Date.now()-30*86400000).slice(0,6).map(h=>{
   const a=state().anime.find(x=>x.id===h.id);if(!a)return null;
   return {title:String(a.title||'').slice(0,180),episode:Math.max(1,Number(h.episode)||1),at:String(h.date||'').slice(0,40)};
  }).filter(Boolean):[];
 ...
 function statsPage(){
  ...
  const months=Array.from({length:12},(_,i)=>{const d=new Date(now.getFullYear(),now.getMonth()-11+i,1),end=new Date(d.getFullYear(),d.getMonth()+1,1);return {label:d.toLocaleDateString('sq-AL',{month:'short'}),n:logs.filter(e=>e.at>=d.getTime()&&e.at<end.getTime()).length}});
  ...
  return '<section class="at-h2-stats-top"><div><span class="pro-eyebrow">MY WATCHING JOURNEY</span><h3>📊 Statistikat e mia</h3><p>Progresi dhe aktiviteti personal, në një vend.</p></div><button type="button" class="pro-btn" data-pro-action="profile-tab" data-id="overview">← Profili</button></section>'+
   ...
   '<div class="at-profile-columns"><section class="pro-panel"><span class="pro-eyebrow">KËTË JAVË</span><h3>7 ditët e javës</h3>'+chart(days,'Episode në ditë')+'</section><section class="pro-panel"><span class="pro-eyebrow">RITMI YT</span><h3>12 muajt e fundit</h3>'+chart(months,'Episode në muaj')+'</section></div>'+
   ...
   '<section class="pro-panel"><span class="pro-eyebrow">ANIME TASTE</span><h3>Top zhanret</h3><div class="at-profile-genres">'+(a.genres.map(([g,n])=>'<div class="at-genre-row"><div><b>'+esc(g)+'</b><small>'+n+'</small></div><span><i style="width:'+Math.round(n/a.maxGenre*100)+'%"></i></span></div>').join('')||'<p class="pro-muted">Shto anime për të parë zhanret.</p>')+'</div></section>'+
 ...
 function render(){
  ...
  const socialStrip=`<section class="at11-profile-social" aria-label="Komuniteti"><button type="button" data-pro-page="friends"><strong>${social.accepted}</strong><span>Miq</span></button><button type="button" data-pro-page="friends"><strong>${social.pending}</strong><span>Kërkesa</span></button><button type="button" data-pro-page="friends"><strong>${social.outgoing}</strong><span>Në pritje</span></button><button type="button" class="at11-find-friends" data-pro-page="friends"><strong>＋</strong><span>Gjej shokë</span></button></section>`;
  ...
  // Mobile-only showcase: covers and counts are derived from this user's own library.
  const showcasePosters=(items,empty)=>items.length?items.slice(0,8).map(x=>{const src=poster(x.cover);return `<button type="button" class="at1161-poster" data-pro-action="profile-anime" data-id="${esc(x.id)}" aria-label="Hap ${esc(x.title)}">${src?`<img src="${esc(src)}" loading="lazy" referrerpolicy="no-referrer" alt="${esc(x.title)}">`:'<span>✦</span>'}<span class="at1161-poster-title">${esc(x.title)}</span></button>`}).join(''):`<p class="at1161-empty">${empty}</p>`;
  const shows=a.anime.filter(x=>!ctx.isMovie(x)),movies=a.anime.filter(x=>ctx.isMovie(x));
  const cover=poster(a.favorites[0]?.cover||shows[0]?.cover||movies[0]?.cover||'');
  const mobileShowcase=`<section class="at1161-profile" aria-label="Profili personal"><div class="at1161-cover">${cover?`<img src="${esc(cover)}" alt="" aria-hidden="true" referrerpolicy="no-referrer">`:''}<div class="at1161-cover-actions"><button type="button" data-pro-page="notifications" aria-label="Njoftimet">♧</button><span>ANIMETRACK · PROFILI IM</span><button type="button" data-pro-action="profile-tab" data-id="settings" aria-label="Ndrysho profilin">⚙</button></div></div><div class="at1161-identity"><div class="at1161-avatar">${avatar?`<img src="${esc(avatar)}" alt="Foto profili" referrerpolicy="no-referrer">`:esc(p.avatar_emoji||'✦')}</div><h2>${esc(name)}</h2><span>${esc(handle)}</span><p>${esc(p.bio||'Historia ime anime, episod pas episodi.')}</p><small>${esc(status)}</small></div><div class="at1161-level"><span aria-hidden="true">✦</span><div><strong>Anime Journey · të përfunduara</strong><div class="at1161-meter"><i style="width:${Math.round(s.stats.completed/Math.max(1,s.stats.titles)*100)}%"></i></div></div><b>${s.stats.episodes.toLocaleString('sq-AL')} EP</b></div><div class="at1161-metrics"><div><strong>${s.stats.episodes.toLocaleString('sq-AL')}</strong><span>Episode</span></div><div><strong>${movies.length}</strong><span>Filma</span></div><div><strong>${(a.minutes/60).toFixed(1)}h</strong><span>Kohë e përafërt</span></div></div><div class="at1161-metrics at1161-social"><button type="button" data-pro-page="friends"><strong>${social.accepted}</strong><span>Miq</span></button><button type="button" data-pro-page="friends"><strong>${social.pending}</strong><span>Kërkesa</span></button><button type="button" data-pro-page="friends"><strong>＋</strong><span>Gjej shokë</span></button></div><div class="at1161-collection"><div class="at1161-section-heading"><h3>Serialet e mia</h3><button type="button" data-mobile-nav="library">Shiko të gjitha (${shows.length}) ›</button></div><div class="at1161-posters">${showcasePosters(shows,'Shto një serial për ta shfaqur këtu.')}</div><div class="at1161-section-heading"><h3>Filmat e mi</h3><button type="button" data-mobile-nav="library">Shiko të gjitha (${movies.length}) ›</button></div><div class="at1161-posters">${showcasePosters(movies,'Filmat e tu do të shfaqen këtu.')}</div></div></section>`;
