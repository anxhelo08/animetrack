...
let accountMode='guest',accountUser=null,cloudClient=null,cloudTimer=null,cloudDirty=false,cloudSaving=false,cloudLastSync='',cloudConnected=false,accountBusy=false,cloudRevision='';
...
function quickLastWatched(id){const a=state.anime.find(x=>x.id===id);if(!a)return null;for(let i=state.history.length-1;i>=0;i--){const e=state.history[i];if(e.id!==id||!['watched','unwatched','season-watched','season-unwatched'].includes(e.action))continue;if(e.action!=='watched')return null;const season=a.seasons.find(x=>x.id===e.seasonId),n=Number(e.episode);return season&&season.watched.includes(n)?{seasonId:season.id,n}:null}return null}
function quickUndoLast(id){const last=quickLastWatched(id);if(!last){notify('Nuk ka episod të fundit për t’u zhbërë.');return false}return updateSeasonEpisode(id,last.seasonId,last.n,false)}
...
async function accountPush(showResult=true){if(accountMode!=='cloud'||!accountUser||cloudSaving)return;clearTimeout(cloudTimer);const uid=accountUser.id;const payload=JSON.parse(JSON.stringify(state));cloudSaving=true;try{let saved,error;
if(cloudRevision){({data:saved,error}=await accountInitClient().from('anime_libraries').update({payload}).eq('user_id',uid).eq('updated_at',cloudRevision).select('updated_at').maybeSingle());}
else {({data:saved,error}=await accountInitClient().from('anime_libraries').insert({user_id:uid,payload}).select('updated_at').single());}
if(error)throw error;
if(!saved)throw Error('Konflikt sinkronizimi: biblioteka ndryshoi në një pajisje tjetër. Eksporto kopjen lokale përpara se të shkarkosh cloud.');
if(accountUser?.id!==uid)return;cloudRevision=saved.updated_at||cloudRevision;cloudConnected=true;cloudLastSync=new Date().toLocaleString('sq-AL');const changed=JSON.stringify(state)!==JSON.stringify(payload);cloudDirty=changed;if(changed)cloudTimer=setTimeout(()=>accountPush(false),1200);if(showResult)accountStatus('Ruajtur në cloud ✓ · '+cloudLastSync,'ok')}catch(e){if(accountUser?.id===uid){cloudConnected=false;cloudDirty=true;accountStatus('Nuk u sinkronizua: '+e.message+' · Ruajtja lokale për llogarinë tënde mbetet.','error');if(showResult)notify('Cloud nuk u lidh. Provo përsëri.')}}finally{cloudSaving=false;accountUI()}}
async function accountPull(manual=false){if(accountMode!=='cloud'||!accountUser)return;if(cloudDirty){if(manual&&!confirm('Ke ndryshime lokale që nuk janë sinkronizuar. Të shkarkosh cloud mund t’i zëvendësojë. Vazhdo?'))return;if(!manual)return;}const uid=accountUser.id;try{const {data,error}=await accountInitClient().from('anime_libraries').select('payload,updated_at').eq('user_id',uid).maybeSingle();if(error)throw error;if(accountUser?.id!==uid)return;if(data?.payload){state=accountNormalizePayload(data.payload);localStorage.setItem(KEY,JSON.stringify(state));cloudDirty=false;cloudConnected=true;cloudLastSync=new Date(data.updated_at).toLocaleString('sq-AL');cloudRevision=data.updated_at||'';cloudLastPullAt=Date.now();accountRefreshViews();if(manual)accountStatus('Biblioteka u rifreskua nga cloud ✓','ok')}else if(manual)accountStatus('Ende nuk ka bibliotekë të ruajtur në cloud.','ok')}catch(e){cloudConnected=false;accountUI();if(manual)accountStatus('Rifreskimi dështoi: '+e.message,'error')}}
