...
test('PWA update notification checks new workers and avoids reload during unsaved cloud writes',()=>{
 ...
 assert.match(css,/\.at-pwa-update/);assert.match(sw,/animetrack-shell-v1191-1/);
});
...
test('10.9 push is explicitly opt-in and staged server secrets never ship in the browser',()=>{
 ...
 assert.match(sw,/animetrack-shell-v1191-1/);
...
test('11.4.1 unwatched is neutral, watched history is green, upcoming cannot be marked',async()=>{
 ...
 await feed.action('tab','upcoming');html=feed.render();assert.match(html,/at117-upcoming-action/);assert.doesNotMatch(html,/data-ios-action="advance"/);assert.match(html,/Episodet që po vijnë/);
});
...
test('11.4.1 mobile polish is scoped to small screens and keeps iOS status bar clear',()=>{
 ...
 assert.match(css,/at114-check\.pending/);assert.match(css,/at114-check\.done/);assert.match(css,/at114-upcoming-intro/);assert.match(css,/display-mode:standalone/);assert.match(css,/safe-area-inset-top/);assert.match(sw,/animetrack-shell-v1191-1/);
});
...
test('11.6 signup and login implement backend responses independently, including pending confirmation',async()=>{
 ...
 const sandbox={window:{ATMobile113:{signupReady:()=>mode,signup:()=>{mode=true}}},location:{protocol:'https:',origin:'https://animetrack.example',pathname:'/'},Date,
  $:elem,accountBusy:false,accountInitClient:()=>client,accountRedirectURL:()=> 'https://animetrack.example/',accountStatus:(msg,kind)=>messages.push([msg,kind]),accountOpenCloud:async()=>{signedIn=true},accountToggle:()=>{},accountName:()=> 'Friend',notify:()=>{},console};
...
test('11.6.2 signup uses canonical production redirect and handles consumed links safely',()=>{const src=fs.readFileSync(path.join(root,'assets/app.js'),'utf8');const html=fs.readFileSync(path.join(root,'index.html'),'utf8');assert.match(src,/ANIMETRACK_AUTH_REDIRECT='https:\/\/animetrack-flax\.vercel\.app\/'/);assert.match(src,/emailRedirectTo:accountRedirectURL\(\)/);assert.match(src,/redirectTo:accountRedirectURL\(\)/);assert.match(src,/otp_expired/);assert.match(src,/provo Hyr/);assert.match(html,/11\.9\.1/)});
...
test('11.8.1 TV resume, bulk prior episodes, season all and grouped Dexter',()=>{
 ...
 assert.match(tv.render(),/Universi Dexter/i);
...
test('11.9 unified library: TV totals, progress and content switch without changing anime',()=>{
 ...
 assert.match(sw,/pro-unified-119\.js/);assert.match(sw,/animetrack-shell-v1191-1/);
});

test('11.9.1 single catalog searches anime and TV, opens TV from results and retains one library',()=>{
 const app=fs.readFileSync(path.join(root,'assets/app.js'),'utf8'),pro=fs.readFileSync(path.join(root,'assets/pro-features.js'),'utf8'),tv=fs.readFileSync(path.join(root,'assets/pro-tv-118.js'),'utf8'),mobile=fs.readFileSync(path.join(root,'assets/pro-mobile-113.js'),'utf8'),unified=fs.readFileSync(path.join(root,'assets/pro-unified-119.js'),'utf8'),html=fs.readFileSync(path.join(root,'index.html'),'utf8');
 ...
 assert.match(app,/if\(item\.kind==='tv'\)/);
 assert.match(pro,/modules\.tv\.openExternal/);
 assert.match(tv,/async function openExternal/);
 assert.match(mobile,/title\.textContent='Biblioteka ime'/);
 assert.match(unified,/title\.textContent='Biblioteka ime'/);
 assert.match(html,/KËRKIM ONLINE • ANIME \+ SERIALE TV/);
 assert.doesNotMatch(pro,/data-pro-page="tv">\\+ Shto serial TV/);
});
